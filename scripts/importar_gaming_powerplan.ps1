Write-Host "Creant i activant el pla d'energia per a gaming..." -ForegroundColor Cyan

# Nom i descripció del pla
$powerPlanName = "SOPOptimizer - Gaming"
$powerPlanDesc = "Pla optimitzat per a gaming."
...
if ($existingPlan) {
    if ($existingPlan -match "([a-fA-F0-9\-]{36})") {
        ...
        Write-Host "Pla d'energia per a gaming ja existent activat." -ForegroundColor Green
    } else {
    ...
    if ($guid -match "([a-fA-F0-9\-]{36})") {
        ...
        powercfg -setactive $guid
        Write-Host "Pla d'energia per a gaming creat i activat correctament." -ForegroundColor Green
    } else {
        Write-Host "No s'ha pogut crear el pla d'energia per a gaming." -ForegroundColor Red
    }
}
